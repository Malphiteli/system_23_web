<template v-slot:default>
	<div :class="['content',isCollapse?'menu--fold':'menu--unfold']">
		...
		<div class="menuLeft">
			...
			<el-menu active-text-color="#fff" background-color="#263238" class="el-menu-vertical-demo"
				...
				<template v-for="(item,index) in menus">
					...
					<el-menu-item v-if="!item.submenu" :index="item.routerName" @click="$router.push({ name: item.routerName })"
		...
		<div class="content-main">
			<div class="navTop horizontalView  top_header" >
        <div class="navTop horizontalView">
          <div>
            <SvgIcon :name="isCollapse?'expand':'fold'" class="icon-svg" @click="isCollapse=!isCollapse" />
          </div>
          <el-breadcrumb separator="/">
            <el-breadcrumb-item v-if="!breadcrumbList.size && breadcrumbList[0]!='设备管理'" :to="{ name: 'index' }">
              设备管理
            </el-breadcrumb-item>
        ...
        <div class="right-btn">
          <el-button type="success" plain @click="backLogin" v-if="flag">退出登录</el-button>
          <el-button type="success" plain  v-else @click="this.$router.push('/login')">登录系统</el-button>
        </div>
			</div>
  ...
  import { useRoute } from 'vue-router'
	...
	export default {
		...
		data: function() {
			return {
				...
				breadcrumbObj: {},
				breadcrumbList: [],
        flag:localStorage?.Authorization!=='null'
			}
		},
		created() {
			let that = this;
    ...
    mounted() {
      ...
      GetUserInfo().then(res=>{
        console.log(res);
        that.menus = res.items;
        that.longTitle = res.longTitle;
        that.littleTitle = res.littleTitle;
        if(localStorage.Authorization === null || localStorage.Authorization === '' || localStorage.Authorization ==='null'){
          this.$router.push('/login')
        }
      })
    }
