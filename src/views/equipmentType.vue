<template>
  <div>
    <el-button type="primary" @click="handClickModal">新增类型</el-button>
  </div>
  ...
  <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-input v-model="type_up" clearable placeholder="类型"></el-input>
        <el-input v-model="description_up" clearable placeholder="描述"></el-input>
...
import {EquipmentTypeAdd, EquipmentTypeDelete, EquipmentTypeList, EquipmentTypeUpdate} from "../utils/api";
...
export default {
  ...
  methods: {
    ...
    addEquipmentType() {
      EquipmentTypeAdd(this.type,this.description).then(res=>{
        alert(res.message);
        if (res.code === true) {
          this.$router.go(0)
        }
    ...
    deleteEquipmentType(){
      EquipmentTypeDelete(this.id_delete).then(res=>{
        if (res.code === true) {
          this.$router.go(0)
        }
      })
    ...
    updateEquipmentType(event) {
      EquipmentTypeUpdate(this.id_up,this.type_up,this.description_up).then(res=>{
        alert(res.message);
        if (res.code === true) {
          this.$router.go(0)
        }
